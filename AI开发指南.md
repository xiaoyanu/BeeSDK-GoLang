...
```text
1. 使用 gofmt 格式化 Go 文件。
2. 在 Windows 运行 build.bat。
3. 检查生成 DLL 为 PE32/i386，并核对 11 个 Bee 中文导出。
```

完成标准：

- 生成 `BeeGoPlugin.dll`。
- `file` 显示 `PE32`、`Intel 80386`。
- 11 个 Bee 中文导出均存在。
- 模拟私聊测试输出 `PRIVATE_HI_REPLY_OK`。
- 检查 DLL 导入表，不允许出现项目私有 DLL。
- 最终在真实 Bee 框架载入测试后，才能声称兼容验证成功。
